...
class DonorServices {
  static Future<Donation?> addDonationItem({
    ...
    final cloudinary = CloudinaryPublic(Secrets.cloudName, Secrets.uploadPreset);
    // CloudinaryPublic("", "");
    try {
      CloudinaryResponse response = await cloudinary.uploadFile(CloudinaryFile.fromFile(image.path, folder: "Serve-Surplus"));
      imageUrl = response.secureUrl;
      Donation donation = Donation(food: food, quantity: quantity, image: imageUrl);
      return donation;
    } catch (error) {
      if (context.mounted) {
  ...
  static void addDonation({
    ...
    try {
      ...
      List<Map<String, dynamic>> donationMaps = donations.map((donation) => donation.toMap()).toList();
  ...
  static Future<List<Donation>> getDonationHistory({required BuildContext context}) async {
      ...
      if (context.mounted) {
        httpResponseHandler(
          ...
          onSuccess: () {
            ...
            donations = (donationHistory as List<dynamic>).map((d) => Donation.fromMap(d)).toList();
  ...
  static Future<List<Donation>> getLiveDonations({required BuildContext context}) async {
    List<Donation> donations = [];
    String token = Provider.of<UserProvider>(context, listen: false).user["token"];
    if (token == '') {
      return donations;
      ...
      if (context.mounted) {
        httpResponseHandler(
          ...
          onSuccess: () {
            ...
            donations = (liveDonations as List<dynamic>).map((d) => Donation.fromMap(d)).toList();
          },
        );
      }
    } catch (error) {
  ...
  static Future<List<Order>> getDonorOrders({required BuildContext context, String? status}) async {
    List<Order> orders = [];
    try {
      String token = Provider.of<UserProvider>(context, listen: false).user["token"];
      String getDonorOrdersUrl = "https://serve-surplus.onrender.com/api/donor/orders";
      ...
      if (context.mounted) {
        httpResponseHandler(
          ...
          onSuccess: () {
            ...
            orders = (donorOrders as List<dynamic>).map((d) => Order.fromMap(d)).toList();
          },
        );
      }
    } catch (error) {
  ...
  static Future<bool> confirmOrder({
    ...
    Map<String, dynamic> requestBody = {"orderId": orderId, "donorId": donorId, "receiverId": receiverId, "secret": secret};
    try {
      String token = Provider.of<UserProvider>(context, listen: false).user["token"];
